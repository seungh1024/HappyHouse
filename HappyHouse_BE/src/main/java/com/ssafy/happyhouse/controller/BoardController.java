...
public class BoardController {
	...
	@ResponseBody
	public ResponseEntity<?> retrieveBoard(@PathVariable("page") int page) throws Exception {
		logger.debug("retrieveQna - 호출");

		System.out.println("board list start: " + page);

		int start = page * 10 - 10;
		List<Board> qnas = boardService.retrieveBoard(start, 10);
		if (qnas.isEmpty()) {
			return new ResponseEntity(HttpStatus.NO_CONTENT);
		}
		return new ResponseEntity<List<Board>>(qnas, HttpStatus.OK);
	}
	...
	public ResponseEntity<Integer> totalBoard() throws Exception {
		...
		return new ResponseEntity<Integer>(boardService.getNumberofBoard(), HttpStatus.OK);
	}
...
//	}
	...
	public ResponseEntity<Board> detailBoard(@PathVariable("articleno") int articleno) {
